...
| `roles/pharos_deploy` | `pharos_runtime` | resolves the newest staged `pharos_*.deb`, installs it on the target host, stops `pharos.service` and `pharos-ha.service` before bundled dynamic-app migrations for packaged apps already installed under `/srv/pharos/apps`, restarts services after migration, and prunes older runtime packages |
