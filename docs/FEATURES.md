...
| `release.yaml` | `localhost` | build Lantern native binaries and packages via Docker Compose across 7 targets (linux-x86_64-gnu [Debian/Ubuntu/RHEL/SUSE], linux-x86_64-musl [Alpine], linux-aarch64-gnu [ARM64], linux-riscv64-gnu, macos-universal, windows-x86_64-msvc, windows-arm64-msvc), optionally scoped to a single target with `-e target=linux-aarch64-gnu`; `.deb` produced by the Docker pipeline for linux-gnu targets; package payload includes only `atlas_studio` and `graph_studio` | `lantern_release` | `lantern_release` |
