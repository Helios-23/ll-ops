...
| `roles/pharos_app_deploy` | `pharos_app` | builds a finalized app root on the controller with `pharos build app --packaging`, reuses or rebuilds the host-native docs renderer only when the native `dev_docs` renderer inputs changed or the binary is missing, extracts the bundle into `/srv/pharos/apps/<app_id>`, and prunes old staged bundles |
