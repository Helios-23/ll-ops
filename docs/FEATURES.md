...
| `build.yml` | `localhost` | uses `../pharos/VERSION` as the automation-facing release version surface, enforces sync with the Pharos root `Cargo.toml` workspace package version, optionally bumps both when `pharos_build_release_version` is set to a newer value, then builds Pharos release artifacts via Docker Compose, optionally for one target; the default all-target build currently skips `macos-universal` because the current cross image is broken for that target | `pharos_build` | `pharos_build` |
...
| `roles/pharos_app_deploy` | `pharos_app` | builds a finalized app root on the controller with `pharos build app --packaging`, ensures the host-native `bin/pharos` CLI and helper binaries exist for app packaging, runs the `dev_docs` renderer fingerprint and `scripts/build_docs.sh` prebuild path only when `app_id=dev_docs`, extracts the bundle into `/srv/pharos/apps/<app_id>`, runs `pharos migrate apply app` on the target only when the deployed manifest host profile is `dynamic-app` using the deployed `app.conf` backend and `/etc/pharos/pharos.conf`, and prunes old staged bundles |
| `roles/pharos_build` | `pharos_build` | treats `../pharos/VERSION` as the automation-facing version surface, requires it to match `../pharos/Cargo.toml` `[workspace.package].version`, optionally bumps both when `pharos_build_release_version` is newer, and builds release artifacts through `pharos/cross/docker`; default target is `all`, override with `-e target=<name>`; `macos-universal` is currently blocked explicitly because the shipped cross-image `libpq.a` for that target contains ELF objects and fails to link as a macOS archive |
| `roles/pharos_deploy` | `pharos_runtime` | resolves the newest staged `pharos_*.deb`, installs it on the target host, optionally preserves every existing `/srv/pharos/apps/*` app bundle with `pharos_deploy_preserve_existing_apps=true`, runs bundled dynamic-app migrations only in the default app-updating mode, restarts services after install, and prunes older runtime packages |
