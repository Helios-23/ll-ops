...
| `lantern-app-deploy.yml` | `gex0` | sync the Lantern repo on the controller, archive `lantern/apps/<app_id>` under `/opt/release/lantern/app` only when the bundle changes, then deploy the newest matching archive into `/srv/lantern/apps/<app_id>` | `lantern_app_deploy` | `lantern_app_deploy` |
...
| `roles/lantern_app_deploy` | `lantern_app_deploy` | Syncs the Lantern repo on the controller, archives a selected app bundle only when the bundle changes, and extracts it into `/srv/lantern/apps/<app_id>` on `gex0` with `lantern:lantern` ownership. |
