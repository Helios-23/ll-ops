...
| `lantern-app-deploy.yml` | `gex0` | sync the Lantern repo on the controller, archive `lantern/apps/<app_id>` under `/opt/release/lantern/app` only when the bundle changes, then deploy the archive to `gex0` only when the controller archive is newer than the cached target archive | `lantern_app_deploy` | `lantern_app_deploy` |
| `lantern-release-deploy.yml` | `localhost`, `gex0` | stage fresh Lantern binaries on the controller, build the `.deb`, then deploy it to `gex0`; package payload includes only `atlas_studio` and `graph_studio` | `lantern_release_build`, `lantern_release_deploy` | `lantern_release_build`, `lantern_release_deploy` |
...
| `roles/lantern_app_deploy` | `lantern_app_deploy` | Syncs the Lantern repo on the controller, archives a selected app bundle only when the bundle changes, and extracts it into `/srv/lantern/apps/<app_id>` on `gex0` with `lantern:lantern` ownership only when the controller archive is newer than the cached target archive. |
| `roles/lantern_deploy` | `lantern_deploy` | Installs a built Lantern `.deb` on `gex0` and starts `lantern.service` and `lantern-ha.service` without removing the staged package from the target host. |
