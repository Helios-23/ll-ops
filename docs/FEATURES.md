...
| `lantern-release-deploy.yml` | `localhost`, `gex0` | stage fresh Lantern binaries on the controller, build the `.deb`, then deploy it to `gex0` | `lantern_release_build`, `lantern_release_deploy` | `lantern_release_build`, `lantern_release_deploy` |
