...
| `roles/lantern_app_deploy` | `lantern_app_deploy` | Syncs the Lantern repo on the controller, renders the selected app into the finalized staged app root at `lantern/dist/apps/<app_id>/` before packaging, archives only that finalized app root, names the archive with the build timestamp plus the 8-char git SHA and the bundle fingerprint short hash, and extracts it directly into `/srv/lantern/apps/<app_id>` on `web0` with `lantern:lantern` ownership whenever the bundle changes or the controller archive basename differs from the deployed archive marker on the target; after extraction the role rewrites the shipped app artifact manifests so `app_root` points at the deployed target path instead of the controller checkout, then verifies both shipped manifests exist at the deployed app root and contain the rewritten target `app_root`. The target caches deployed archives under `/srv/lantern/apps/<app_id>/.archives/` and only updates `.deployed-archive` after a successful extraction. |
